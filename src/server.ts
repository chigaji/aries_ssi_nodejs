...
const config: InitConfig = {
    label: 'Bigster_issuer_agent',
    walletConfig: {
    //   id: 'wallet-id',
      id: 'bigster',
      key: 'bigsteragent0000000000000000000',
    },
  ...
  const agent = new Agent({
    ...
    modules: {
        indyVdr: new IndyVdrModule({
            indyVdr, 
            networks: [{
                indyNamespace: 'bigster:ssi_testnet',
                isProduction: false,
...
agent.registerInboundTransport(new HttpInboundTransport({ port: 6000 }))
...
app.get('/', (req: Request, res: Response) => {
  res.send('Welcome to the Gym Membership API!');
  agent
  .initialize()
  .then(() => {
